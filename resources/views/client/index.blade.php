...
@section('content')
    ...
    <div class="row match-height">
        @foreach($clearances->split($clearances->count()/2) as $row)
            @foreach($row as $clr)
             <div class="col-md-12 col-xl-6">
                <a href="{{ url('user/clearance/'.$clearance['id']) }}" class="card bg-primary text-white clearance">
                    <div class="px-2 pt-2 pb-0">
                        <div class="clearance-id"> {{ $clr->id }} </div>
                        <div class="d-flex justify-content-between">
                            <div>
                                <div class="clearance-title"> {{ $clr->project_title }}  </div>
                                <div class="clearance-location"> {{ $clr->project_location }} </div>
                            </div>
                            <div>
                                <div class="clearance-header">Payables</div>
                                <div class="clearance-payable"> {{ $clr->amount }}  </div>
                            </div>
                        </div>
                    </div>                                       
                    <hr/>
                    <div class="px-2 pt-0 pb-2 d-flex">
                        <div><span class="font-weight-bold">FSEC</span> <span class="badge badge-pill badge-success ml-25"> {{  $clr['fsec'] }}</span> </div>
                        <div class="ml-1"><span class="font-weight-bold">FSIC</span> <span class="badge badge-pill badge-secondary ml-25"> {{  $clr['fsic'] }}</span> </div>
                    </div>             
                </a>
            </div> 
             @endforeach 
